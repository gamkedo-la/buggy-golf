...
public class SoundManager : MonoBehaviour {
    ...
    void Start () {
        ...
        musicSource = CameraBuggyFollowOrbit.Instance.GetComponent<AudioSource>();
    ...
    void Update()
    {
        if (isCarMuted)
        {
            SoundManager.Instance.revSource.volume = 0.0f;
            boostSource.loop = false;
        }
        else {
            SoundManager.Instance.revSource.volume = Mathf.Abs(Input.GetAxis("Vertical")) * 0.4f * masterVol * soundVol;
